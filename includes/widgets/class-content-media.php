...
final class Content_Media extends Base {
	...
	protected function render(): void { $s = $this->get_settings_for_display(); $has_image = ! empty( $s['image']['id'] ) || ! empty( $s['image']['url'] ); $content = do_shortcode( wpautop( (string) $s['content'] ) ); ?><div class="agris-content-media is-<?php echo esc_attr( $s['image_side'] ); ?> <?php echo $has_image ? 'has-media' : 'has-no-media'; ?>"><div><?php $this->render_heading( $s ); ?><div class="agris-richtext"><?php echo wp_kses_post( $content ); ?></div></div><?php if ( $has_image ) : ?><div class="agris-media-frame"><?php echo Group_Control_Image_Size::get_attachment_image_html( $s, 'image', 'image' ); ?></div><?php endif; ?></div><?php }
}
