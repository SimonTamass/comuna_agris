...
final class Cta_Banner extends Base {
	...
	protected function render(): void { $s = $this->get_settings_for_display(); ?><section class="agris-cta"><div><?php $this->render_heading( $s ); ?><?php if ( $s['button_text'] ) : ?><a class="agris-button agris-button-light" <?php echo self::link_attrs( $s['button_link'] ); ?>><?php echo esc_html( $s['button_text'] ); ?></a><?php endif; ?></div><?php if ( $s['image']['url'] ) : ?><div class="agris-cta-image"><?php echo Group_Control_Image_Size::get_attachment_image_html( $s, 'image', 'image' ); ?></div><?php endif; ?></section><?php }
}
