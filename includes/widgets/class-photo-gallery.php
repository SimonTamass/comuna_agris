...
final class Photo_Gallery extends Base {
	...
	protected function render(): void { $s = $this->get_settings_for_display(); ?><section class="agris-gallery-widget"><?php $this->render_heading( $s ); ?><div class="agris-gallery agris-grid-<?php echo esc_attr( $s['columns'] ); ?>"><?php foreach ( $s['items_list'] as $item ) : if ( empty( $item['image']['url'] ) ) continue; ?><a href="<?php echo esc_url( $item['image']['url'] ); ?>" data-agris-lightbox data-agris-lightbox-group="agris-<?php echo esc_attr( $this->get_id() ); ?>" data-agris-lightbox-caption="<?php echo esc_attr( $item['caption'] ); ?>"><img src="<?php echo esc_url( $item['image']['url'] ); ?>" alt="<?php echo esc_attr( $item['caption'] ); ?>" loading="lazy"><?php if ( $item['caption'] ) : ?><span><?php echo esc_html( $item['caption'] ); ?></span><?php endif; ?></a><?php endforeach; ?></div></section><?php }
}
