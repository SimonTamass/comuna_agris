...
Adds 25 purpose-built widgets under the “Comuna Agriș” Elementor category, plus a structured Document post type and document categories.
...
* Manually editable post template with an accessible gallery and document downloads
...
* Automatically restrict hero updates to the homepage's Romanian or Hungarian Polylang language, with configurable count and a manual-list fallback.

= 2.2.6 =
* Prevent the automatic inline PDF-link decorator from modifying structured post-template document cards.
* Keep PDF, DOC and other recognized download cards aligned identically to image attachment cards.

= 2.2.5 =
* Replace Elementor's gallery editor flow with a direct WordPress Media Library picker that supports selecting any number of images in one operation.
* Detect the current post language through Polylang and render untouched post-template labels and placeholder content fully in Romanian or Hungarian.
* Add an explicit language override and language-neutral file-size units while preserving custom text and legacy gallery values.
...
* Preserve rendering compatibility for gallery images saved with the previous repeater control.
* Use Media Library captions and alternative text in the accessible lightbox.

= 2.2.2 =
* Add independent visibility switches for the post template title area, gallery and downloadable documents.
* Add a dedicated lightbox switch and ensure the post template gallery opens in the accessible image viewer.

= 2.2.1 =
* Keep Elementor editor previews on the native WordPress template so the widget panel can initialize normally.
* Remove the duplicate, identifier-less Elementor document wrapper that caused Elementor Pro preview errors.
* Declare compatibility with Elementor 4.2.3 and Elementor Pro 4.2.2.

= 2.2.0 =
* Add a standalone Elementor post-template widget with an editable title, introduction and rich content.
* Add a responsive lightbox gallery and accessible document download cards to the new post template.
...
* Protect search, header and accessibility control focus states from Elementor global button and form styles.
* Improve the mobile search dialog sizing and keyboard-focus treatment.

= 2.1.16 =
* Match the accessibility panel and floating control to the approved local design, including the accessibility icon, heading badge and grouped text-size controls.
* Add localized accessible names to the text-size controls and cache-bust the updated frontend styling.

= 2.1.15 =
* Include descendant categories in dynamic archives so public records are not omitted.
* Recover substantive post content when an imported page and post share the same public slug.
* Restore the dynamic sitemap, both official phone numbers, audited mayor hours and missing official-gazette links.
* Make homepage update, announcement and council-decision sections query current published content.
* Restore the council declaration link found during the full content audit.

= 2.1.14 =
* Redirect legacy WordPress attachment pages to their original media files so gallery images no longer open outside the shared widget system.

= 2.1.13 =
* Correct public-information, official-gazette, gallery and legacy PDF links found during the live dev audit.
* Prevent shared archive and single-post title bands from creating horizontal page overflow.
* Add a secure POST fallback to the AJAX contact form.

= 2.1.12 =
* Restore the approved local redesign imagery through editable hero widget controls.
* Add complete widget compositions for the municipality, leadership, local council, public information, official gazette and contact pages while preserving the existing WordPress URLs.
* Align shared Romanian header and footer identity text with the bilingual local mockups.

= 2.1.11 =
* Remove the live contact-page PHP fallback from 2.1.10 so regular pages render safely again; contact pages still rebuild through the Elementor template tool.
* Keep the safer frontend lightbox initialization for WordPress article images.
