#!/usr/bin/env python3
"""Compare the public and development Comuna Agriș WordPress sites.
...
PROD_ORIGIN = "https://comunaagris.ro"
DEV_ORIGIN = "https://dev.comunaagris.ro"
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "output" / "live-content-audit"
USER_AGENT = "Mozilla/5.0 (compatible; Codex-ComunaAgris-Content-Audit/1.0)"
MAX_WORKERS = 10
TIMEOUT_SECONDS = 45
RETRIES = 3
...
def slice_frontend_content(document: str, environment: str) -> str:
    ...
    if environment == "prod":
        ...
        end_pattern = r'<footer\b[^>]*\bclass=["\'][^"\']*\bagris-footer\b'
...
def normalize_text(value: str) -> str:
    ...
    value = re.sub(r"https?://(?:dev\.)?comunaagris\.ro", " comunaagris ", value)
...
def content_link_key(value: str) -> str | None:
    ...
    if host not in {"comunaagris.ro", "www.comunaagris.ro", "dev.comunaagris.ro"}:
        return None
...
def write_reports(
    ...
    lines = [
        "# Comuna Agriș – publikus és fejlesztői tartalmi audit",
        "",
